...
public abstract class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		p_order = (MOrder) MTable.get(Env.getCtx(), MOrder.Table_ID).getPO(C_Order_ID, getTrxName());    //  save
	...
	protected Vector<Vector<Object>> getRMAData(int M_RMA_ID)
	{
		m_invoice = null;
		p_order = null;
		m_rma = (MRMA) MTable.get(Env.getCtx(), MRMA.Table_ID).getPO(M_RMA_ID, getTrxName());
	...
	{
		m_invoice = (MInvoice) MTable.get(Env.getCtx(), MInvoice.Table_ID).getPO(C_Invoice_ID, getTrxName()); // save
		p_order = null;
		m_rma = null;
