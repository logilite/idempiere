...
public class DPFavourites extends DashboardPanel implements EventListener<Event>
{
	...
	private boolean					isDefaultTreeWriteAccess	= false;
	...
	public DPFavourites()
	{
		...
		btnCopyTree = new ToolBarButton("CopyTree");
		btnCopyTree.setClass("fav-button-copytree");
		...
		btnShowMyTree = new ToolBarButton("ShowMyTree");
		btnShowMyTree.setClass("fav-button-mytree");
		...
		btnShowDefaultTree = new ToolBarButton("ShowDefaultTree");
		btnShowDefaultTree.setClass("fav-button-defaulttree");
		...
		toolbar.appendChild(btnCopyTree);
		toolbar.appendChild(btnShowMyTree);
	 ...
	 * @param isReload
	 */
	public void initTreeModel(boolean isReload)
	{
		treeModel = FavoriteSimpleTreeModel.initADTree(tree, AD_User_ID, isReload);
	} // initTreeModel
	...
	public void onEvent(Event event)
	{
		...
		if (eventName.equals(Events.ON_CLICK))
		{
			...
			else if (event.getTarget() == btnCopyTree)
			{
				...
				MTreeFavorite.cloneTreeStructure(defFavTreeID, roleID, AD_User_ID, null);
		...
		else if (eventName.equals(Events.ON_SELECT) && event.getTarget() == tree)
		{
			...
			btnEdit.setDisabled(mtNode.getNode_ID() == 0 || !mtNode.isSummary() || (!(isDefaultTreeWriteAccess || !isDefaultTree)));
		}
		else if (eventName.equals(Events.ON_DROP) && event instanceof DropEvent && event.getTarget() instanceof Panelchildren)
		{
			...
			if (de.getDragged() != de.getTarget())
			{
				if (de.getDragged() instanceof Treerow)
				{
					...
					int treeFavoriteID = MTreeFavorite.getFavoriteTreeID(AD_User_ID, Env.getAD_Role_ID(Env.getCtx()));
					//
	...
	private void doExpandCollapseAll()
	{
		...
		if (!tree.getChildren().isEmpty())
		{
			...
			{
				btnExpand.setSclass("fav-expand-mode");
