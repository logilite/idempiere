...
public class WWFActivity extends ADForm implements EventListener<Event>
{
	...
	public void onOK()
	{
		...
		try {
			...
			// then we have to assign the user manually because the user assignment dialog isn’t opening. TODO: fix this in the future.
			MWFResponsible resp = m_activity.getResponsible();
			boolean isForwordMandatory = (node.isUserChoice() || node.isUserTask()) && (resp != null && resp.isManual());
			if (forward != null)
			{
				if (log.isLoggable(Level.CONFIG)) log.config("Forward to " + forward);
				int fw = ((Integer)forward).intValue();
				
				if (isForwordMandatory)
				{
					if (fw == 0)
					{
