...
public class Chosenbox<T> extends Div {
	...
	public void service(org.zkoss.zk.au.AuRequest request, boolean everError) {
		...
		if (cmd.equals(Events.ON_SELECT)) {
			...
			for (int i = 0; i < selItems.size(); i++) {
				int idx = 0;
				String selItemValue = selItems.get(i).toString();
				if (selItemValue.contains("&lt;") && selItemValue.contains("&gt;"))
					selItemValue = html2Text(selItemValue);
				idx = getIndexFromValue(selItemValue, true);
				_selIdxs.add(idx);
	...
	public String html2Text(String html)
	{
		...
		try
		{
			...
			text = doc.getText(1, doc.getLength()-1);
		}
		catch (IOException e)
		{
			e.printStackTrace();
