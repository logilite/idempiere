...
public class MMovement extends X_M_Movement implements DocAction,DocOptions
{
			...
			try
			{
					...
					&& product.isStocked() )
				{
					...
					if (line.getM_AttributeSetInstance_ID() > 0 && qtyToAllocate.signum() != 0)
					{
						...
						if (qtyToAllocate.compareTo(Env.ZERO) > 0) {
						...
						for (MStorageOnHand storage : storages) {
							if (storage.getQtyOnHand().compareTo(qtyToAllocate) >= 0) {
						...
						trxFrom = new MTransaction (getCtx(), line.getAD_Org_ID(), 
							...
							qtyToAllocate.negate(), getMovementDate(), get_TrxName());
						trxFrom.setM_MovementLine_ID(line.getM_MovementLine_ID());
						if (!trxFrom.save())
						{
							m_processMsg = "Transaction From not inserted (MA) [" + product.getValue() + "] - ";
						...
						MTransaction trxTo = new MTransaction (getCtx(), line.getAD_Org_ID(), 
							...
							qtyToAllocate, getMovementDate(), get_TrxName());
						trxTo.setM_MovementLine_ID(line.getM_MovementLine_ID());
						if (!trxTo.save())
						{
	...
	public boolean reActivateIt()
	{
		...
		for (MMovementLine line : lines)
		{
			...
			try
			{
				...
				{
					BigDecimal qtyToAllocate = line.getMovementQty();
					for (MMovementLineMA ma : MMovementLineMA.get(getCtx(), line.getM_MovementLine_ID(), get_TrxName()))
					{
						// Update Locator To Product movement qty
						MLocator locatorTo = new MLocator(getCtx(), line.getM_LocatorTo_ID(), get_TrxName());
						if (!MStorageOnHand.add(getCtx(), locatorTo.getM_Warehouse_ID(), line.getM_LocatorTo_ID(), line.getM_Product_ID(),
												ma.getM_AttributeSetInstanceTo_ID(), ma.getMovementQty().negate(), ma.getDateMaterialPolicyTo(),
												get_TrxName()))
						{
							m_processMsg = Msg.getMsg(	getCtx(), "InventoryMoveStorageNotCorrect",
														new Object[] { line.getM_Product().getValue(), CLogger.retrieveErrorString("") });
							return false;
						}

						// Update Locator From Product movement qty
						MLocator locator = new MLocator(getCtx(), line.getM_Locator_ID(), get_TrxName());
						if (!MStorageOnHand.add(getCtx(), locator.getM_Warehouse_ID(), line.getM_Locator_ID(), line.getM_Product_ID(),
												ma.getM_AttributeSetInstance_ID(), ma.getMovementQty(), ma.getDateMaterialPolicy(),
												get_TrxName()))
						{
							m_processMsg = Msg.getMsg(	getCtx(), "InventoryMoveStorageNotCorrect",
														new Object[] { line.getM_Product().getValue(), CLogger.retrieveErrorString("") });
						...
						if (!deleteTransactions(line, ma, ma.getMovementQty()))
							return false;

						qtyToAllocate = qtyToAllocate.subtract(ma.getMovementQty());
						if (ma.isAutoGenerated() && !ma.delete(false, get_TrxName()))
						{
							m_processMsg = Msg.getMsg(	getCtx(), "InventoryMoveLineMAFailedToDelete",
														new Object[] { product.getValue(), ma.getM_AttributeSetInstance_ID(), ma.getDateMaterialPolicy() });
							return false;
						}
					}

					// When ASI is present on Line
					if (line.getM_AttributeSetInstance_ID() > 0 && qtyToAllocate.signum() != 0)
					{
						Timestamp dateMPolicy = null;
						MStorageOnHand[] storages = null;
						...
						{
							// Find Date Material Policy bases on ASI
							storages = MStorageOnHand.getWarehouse(	getCtx(), 0, line.getM_Product_ID(), line.getM_AttributeSetInstanceTo_ID(), null,
																	MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), false, line.getM_LocatorTo_ID(),
																	get_TrxName());
						}
						for (MStorageOnHand storage : storages)
						{
						...
						if (dateMPolicy == null && qtyToAllocate.negate().signum() > 0)
							effDateMPolicy = getMovementDate();
						if (!MStorageOnHand.add(getCtx(), locatorTo.getM_Warehouse_ID(), line.getM_LocatorTo_ID(),
												line.getM_Product_ID(), line.getM_AttributeSetInstanceTo_ID(),
												qtyToAllocate.negate(), effDateMPolicy, get_TrxName()))
						{
							m_processMsg = Msg.getMsg(	getCtx(), "InventoryMoveStorageNotCorrect",
														new Object[] { product.getValue(), CLogger.retrieveErrorString("") });
							return false;
						}
						...
						if (dateMPolicy == null && qtyToAllocate.signum() > 0)
							effDateMPolicy = getMovementDate();
						MLocator locator = new MLocator(getCtx(), line.getM_Locator_ID(), get_TrxName());
						if (!MStorageOnHand.add(getCtx(), locator.getM_Warehouse_ID(), line.getM_Locator_ID(),
						...
						// Delete Line Transaction
						if (!deleteTransactions(line, null, qtyToAllocate))
							return false;
					}
				} // product stock
			}
		...
		MFactAcct.deleteEx(MMovement.Table_ID, getM_Movement_ID(), get_TrxName());
		setPosted(false);
		setDocAction(DOCACTION_Complete);
		setProcessed(false);
		setIsApproved(false);
	 ...
	 * @param  ma          Movement Line Allocation if present
	 * @param  movementQty Move Quantity
	 * 	 @return             true if Transaction is delete
	 */
	private boolean deleteTransactions(MMovementLine line, MMovementLineMA ma, BigDecimal movementQty)
	{
		String whereClause = "		M_MovementLine_ID = ? AND MovementType  = ? "
								+ " AND M_Product_ID = ? AND M_AttributeSetInstance_ID = ? "
								+ " AND M_Locator_ID = ? AND MovementQty = ? AND MovementDate = ? ";
		int asiToID = 0;
		int asiFromID = 0;
		if (!Objects.isNull(ma))
		{
			asiToID = ma.getM_AttributeSetInstanceTo_ID();
			asiFromID = ma.getM_AttributeSetInstance_ID();
		}
		else
		{
			asiToID = line.getM_AttributeSetInstanceTo_ID();
			asiFromID = line.getM_AttributeSetInstance_ID();
		}

		// Delete Transaction TO
		Query query = new Query(getCtx(), MTransaction.Table_Name, whereClause, get_TrxName());
		query.setParameters(line.getM_MovementLine_ID(), MTransaction.MOVEMENTTYPE_MovementTo, line.getM_Product_ID(),
							asiToID, line.getM_LocatorTo_ID(), movementQty, getMovementDate());
		query.setClient_ID();
		MTransaction trxTo = query.first();

		if (Objects.isNull(trxTo))
		{
			m_processMsg = Msg.getMsg(getCtx(), "InventoryMoveTransactionToNotFound", new Object[] { line.getM_Product().getValue() });
			return false;
		}
		if (!trxTo.delete(false, get_TrxName()))
		{
			m_processMsg = Msg.getMsg(getCtx(), "InventoryMoveTransactionToFailedToDelete", new Object[] { line.getM_Product().getValue() });
			return false;
		}

		// Delete Transaction From
		query = new Query(getCtx(), MTransaction.Table_Name, whereClause, get_TrxName());
		query.setParameters(line.getM_MovementLine_ID(), MTransaction.MOVEMENTTYPE_MovementFrom, line.getM_Product_ID(),
							asiFromID, line.getM_Locator_ID(), movementQty.negate(), getMovementDate());
		query.setClient_ID();
		MTransaction trxFrom = query.first();

		if (Objects.isNull(trxFrom))
		{
			m_processMsg = Msg.getMsg(getCtx(), "InventoryMoveTransactionFromNotFound", new Object[] { line.getM_Product().getValue() });
			return false;
		}
		if (!trxFrom.delete(false, get_TrxName()))
		{
			m_processMsg = Msg.getMsg(getCtx(), "InventoryMoveTransactionFromFailedToDelete", new Object[] { line.getM_Product().getValue() });
			return false;
		}
		return true;
	} // deleteTransactions
