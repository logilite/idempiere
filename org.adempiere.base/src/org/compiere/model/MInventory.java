	...
	public String prepareIt()
	{
		...
		if(!MDocType.DOCSUBTYPEINV_CostAdjustment.equals(docSubTypeInv))
		{
			...
			String sql = "Select (il.line::text || '-' || p.value) as Description From M_InventoryLine il "
					...
					+ " And Exists (Select 1 From M_InventoryLineMA ma "
					+ " Where ma.M_InventoryLine_ID = il.M_InventoryLine_ID And ma.IsActive = 'Y' And ma.IsAutoGenerated='N')"
			...
			try
			{
				...
				if (sb.length() > 0)
				{
					StringBuilder msg = new StringBuilder("Failed. Error on Lines: [ ");
					m_processMsg = msg.append(sb.toString()).append(" ]")
							.append("  Inventory Line with ASI > 0 can not have Attribute lines.").toString();
					return DOCSTATUS_Invalid;
				}
			}
			catch (Exception e)
			{
				log.severe("Error while validating inventory MA lines ");
			...
			// IF MA line present, then MovementQty on inventory line must match
			// with sum(maQty)
			// If MA line not present then will be created from
			// checkMaterialPolicy()
			// "Inner Join" in below query filters Inventory Lines with no MA
			// Lines
			sql = "Select (iol.line::text || '-' || p.value) as Description from M_InventoryLine iol"
					+ " Inner Join M_InventoryLineMA ma on iol.M_InventoryLine_ID = ma.M_InventoryLine_ID"
			...
			try
			{
				...
				if (sb.length() > 0)
				{
					StringBuilder msg = new StringBuilder(
							"Cannot complete Document. Attribute " + " qty not matched on following Line(s). [ ");
					m_processMsg = msg.append(sb.toString()).append(" ]").toString();
					return DOCSTATUS_Invalid;
			...
			{
				log.log(Level.SEVERE, "Error while validating MA qty");
				throw new AdempiereException(e);
			}
			finally
			{
					...
					if (product != null && product.isASIMandatory(line.isSOTrx()))
					{
						if (!product.getAttributeSet().excludeTableEntry(MInventoryLine.Table_ID, line.isSOTrx()))
						{
							...
							if (MDocType.DOCSUBTYPEINV_PhysicalInventory.equals(docSubTypeInv))
								qtyDiff = line.getQtyBook().subtract(line.getQtyCount());
							// verify if the ASIs are captured on lineMA
							MInventoryLineMA mas[] = MInventoryLineMA.get(getCtx(), line.getM_InventoryLine_ID(),
									get_TrxName());
							BigDecimal qtyma = Env.ZERO;
							for (MInventoryLineMA ma : mas)
							{
