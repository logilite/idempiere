		...
		if (newRecord && getC_InvoiceLine_ID() == 0 && getReversal_ID()==0) 
		{
			...
			for (int i = 0; i < mpi.length; i++) 
			{
						...
						mpi[i].getM_AttributeSetInstance_ID() == getM_AttributeSetInstance_ID()) 
				{
					...
					BigDecimal matchedInvQty = DB.getSQLValueBD(get_TrxName(),
							"Select Sum(Qty) from M_MatchPO Where C_InvoiceLine_ID = ? AND Reversal_ID is null",
							mpi[i].getC_InvoiceLine_ID());
					matchedInvQty = matchedInvQty == null ? Env.ZERO : matchedInvQty;
					
					MInvoiceLine invLine = (MInvoiceLine) MTable.get(getCtx(), MInvoiceLine.Table_ID)
							.getPO(mpi[i].getC_InvoiceLine_ID(), get_TrxName());
					BigDecimal qtyInvoiced = invLine.getQtyInvoiced();
					
					BigDecimal availableQty = qtyInvoiced.subtract(matchedInvQty);
					
					if (availableQty.compareTo(getQty()) == 0)  // same quantity
					{
					...
					{
						MInvoiceLine il = (MInvoiceLine) MTable.get(getCtx(), MInvoiceLine.Table_ID).getPO(mpi[i].getC_InvoiceLine_ID(), get_TrxName());
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (success)
		{
			...
			if (getC_OrderLine_ID() > 0)
			{
				...
				if (validateOrderedQty)
				{
					...
					MOrderLine line = (MOrderLine) MTable.get(getCtx(), MOrderLine.Table_ID).getPO(getC_OrderLine_ID(), get_TrxName());
		...
		if (success && getC_OrderLine_ID() != 0)
		{
			...
			if (orderLine.getM_AttributeSetInstance_ID() == 0 && getM_InOutLine_ID() != 0 && getC_InvoiceLine_ID() != 0)
			{
				...
				// Set PO Line ASI when Qty fully matched with POLine, MRLine, InvLine and MatchPO
				if (iol.getMovementQty().compareTo(orderLine.getQtyOrdered()) == 0 && iol.getMovementQty().compareTo(il.getQtyInvoiced()) == 0 && iol.getMovementQty().compareTo(getQty()) == 0)
			...
			{
				MInOutLine iol = (MInOutLine) MTable.get(getCtx(), MInOutLine.Table_ID).getPO(getM_InOutLine_ID(), get_TrxName());
				if (iol.getMovementQty().compareTo(orderLine.getQtyOrdered()) == 0 && iol.getMovementQty().compareTo(getQty()) == 0)
					orderLine.setM_AttributeSetInstance_ID(iol.getM_AttributeSetInstance_ID());
			}
