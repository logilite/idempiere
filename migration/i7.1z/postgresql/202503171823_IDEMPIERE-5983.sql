...
INSERT INTO AD_Ref_List (AD_Ref_List_ID,Name,AD_Reference_ID,Value,AD_Client_ID,AD_Org_ID,IsActive,Created,CreatedBy,Updated,UpdatedBy,EntityType,AD_Ref_List_UU) VALUES (200716,'No Posting Required',234,'R',0,0,'Y',TO_TIMESTAMP('2025-03-17 19:16:22','YYYY-MM-DD HH24:MI:SS'),100,TO_TIMESTAMP('2025-03-17 19:16:22','YYYY-MM-DD HH24:MI:SS'),100,'D','def573d1-07ae-4b61-8c45-756ae7353596')
;
...
CREATE OR REPLACE VIEW rv_unposted
 ...
 SELECT gl_journal.ad_client_id,
   ...
   FROM gl_journal
  WHERE gl_journal.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND gl_journal.docstatus <> 'VO'::bpchar
UNION
 SELECT pi.ad_client_id,
  ...
  WHERE pi.posted NOT IN ('Y'::bpchar, 'R'::bpchar)
UNION
 SELECT c_invoice.ad_client_id,
   ...
   FROM c_invoice
  WHERE c_invoice.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_invoice.docstatus <> 'VO'::bpchar
UNION
 SELECT m_inout.ad_client_id,
    ...
    m_inout.processedon
   FROM m_inout
  WHERE m_inout.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND m_inout.docstatus <> 'VO'::bpchar
UNION
 SELECT m_inventory.ad_client_id,
   ...
   FROM m_inventory
  WHERE m_inventory.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND m_inventory.docstatus <> 'VO'::bpchar
UNION
 SELECT m_movement.ad_client_id,
   ...
   FROM m_movement
  WHERE m_movement.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND m_movement.docstatus <> 'VO'::bpchar
UNION
 SELECT m_production.ad_client_id,
   ...
   FROM m_production
  WHERE m_production.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND m_production.docstatus::text <> 'VO'::text
UNION
 SELECT c_cash.ad_client_id,
    ...
    c_cash.processedon
   FROM c_cash
  WHERE c_cash.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_cash.docstatus <> 'VO'::bpchar
UNION
 SELECT c_payment.ad_client_id,
   ...
   FROM c_payment
  WHERE c_payment.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_payment.docstatus <> 'VO'::bpchar
UNION
 SELECT c_allocationhdr.ad_client_id,
  ...
  WHERE c_allocationhdr.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_allocationhdr.docstatus <> 'VO'::bpchar
UNION
 SELECT c_bankstatement.ad_client_id,
  ...
  WHERE c_bankstatement.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_bankstatement.docstatus <> 'VO'::bpchar
UNION
 SELECT m_matchinv.ad_client_id,
   ...
   FROM m_matchinv
  WHERE m_matchinv.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND COALESCE(m_matchinv.m_matchinvhdr_id, 0::numeric) = 0::numeric
UNION
 SELECT m_matchinvhdr.ad_client_id,
   ...
   FROM m_matchinvhdr
  WHERE m_matchinvhdr.posted NOT IN ('Y'::bpchar, 'R'::bpchar)
UNION
 SELECT m_matchpo.ad_client_id,
   ...
   FROM m_matchpo
  WHERE m_matchpo.posted NOT IN ('Y'::bpchar, 'R'::bpchar)
UNION
 SELECT c_order.ad_client_id,
    ...
    c_order.processedon
   FROM c_order
  WHERE c_order.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND c_order.docstatus <> 'VO'::bpchar
UNION
 SELECT m_requisition.ad_client_id,
   ...
   FROM m_requisition
  WHERE m_requisition.posted NOT IN ('Y'::bpchar, 'R'::bpchar) AND m_requisition.docstatus <> 'VO'::bpchar;
