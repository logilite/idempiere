...
SET SERVEROUTPUT ON;
DECLARE 
CURSOR cursorRecord IS SELECT DISTINCT ba.AD_Client_ID, ba.C_AcctSchema_ID, vu.Account_ID
...
BEGIN
	...
	EXCEPTION WHEN duplicateAcct THEN
		DBMS_OUTPUT.PUT_LINE('EXCEPTION: Error Bank account found different UnAllocatedCash account used against same client and same acctSchema, Please verify manually to correct action as per ticket on IDEMPIERE-5516.');
END;
...
BEGIN
	FOR item IN cursorRecord 
	LOOP
		DBMS_OUTPUT.PUT_LINE( 'Payment Selection account used, clientID: ' || item.AD_Client_ID || '; acctSchemaID:' || item.C_AcctSchema_ID || ' accountID: ' || item.Account_ID);
	...
	EXCEPTION WHEN duplicateAcct THEN
		DBMS_OUTPUT.PUT_LINE('EXCEPTION: Error Bank account found different Payment Selection account used against same client and same acctSchema, Please verify manually to correct action as per ticket on IDEMPIERE-5516.');
END;
...
MERGE INTO C_BP_Group_Acct bga
    USING 	(SELECT a.AD_Client_ID, a.C_AcctSchema_ID, B_UnallocatedCash_Acct, B_PaymentSelect_Acct
			FROM C_AcctSchema_Default a
			) st ON ( st.AD_Client_ID = bga.AD_Client_ID AND st.C_AcctSchema_ID  = bga.C_AcctSchema_ID )
WHEN MATCHED THEN
UPDATE 
SET bga.B_UnallocatedCash_Acct = st.B_UnallocatedCash_Acct,
  	bga.B_PaymentSelect_Acct   = st.B_PaymentSelect_Acct
WHERE ( bga.B_UnallocatedCash_Acct IS NULL OR bga.B_PaymentSelect_Acct IS NULL )  
;
...
MERGE INTO C_BP_Customer_Acct bga
    USING 	(SELECT a.AD_Client_ID, a.C_AcctSchema_ID, b.C_BPartner_ID, B_UnallocatedCash_Acct
			...
			) st ON ( 		st.AD_Client_ID = bga.AD_Client_ID 
						AND st.C_AcctSchema_ID  = bga.C_AcctSchema_ID 
						AND st.C_BPartner_ID = bga.C_BPartner_ID )
WHEN MATCHED THEN
UPDATE 
SET bga.B_UnallocatedCash_Acct = st.B_UnallocatedCash_Acct
...
-- Jan 5, 2023, 9:22:34 PM IST
UPDATE AD_Field_Trl SET IsActive = 'N' WHERE AD_Field_ID=5135
;

-- Jan 5, 2023, 9:22:34 PM IST
UPDATE AD_Field SET IsActive = 'N' WHERE AD_Field_ID=5135
;

-- Jan 5, 2023, 9:23:03 PM IST
UPDATE AD_Field_Trl SET IsActive = 'N' WHERE AD_Field_ID=5136
;

-- Jan 5, 2023, 9:23:03 PM IST
UPDATE AD_Field SET IsActive = 'N' WHERE AD_Field_ID=5136
;

-- Jan 5, 2023, 9:23:51 PM IST
UPDATE AD_Column_Trl SET IsActive = 'N' WHERE AD_Column_ID=6495
;

-- Jan 5, 2023, 9:23:51 PM IST
UPDATE AD_Column SET IsActive = 'N' WHERE AD_Column_ID=6495
;

-- Jan 5, 2023, 9:24:23 PM IST
UPDATE AD_Column_Trl SET IsActive = 'N' WHERE AD_Column_ID=6494
;

-- Jan 5, 2023, 9:24:23 PM IST
UPDATE AD_Column SET IsActive = 'N' WHERE AD_Column_ID=6494
;
