...
c.ad_chart_id, f.ad_labelstyle_id, f.ad_fieldstyle_id, c.pa_dashboardcontent_id, f.isquickform, COALESCE(f.placeholder, c.placeholder) AS placeholder, c.ishtml, f.entitytype, COALESCE(f.ad_val_rule_Lookup_id, c.ad_val_rule_Lookup_id) AS ad_val_rule_Lookup_id, vrf.code AS validationcodeLookup
   FROM ad_field f
...
SELECT register_migration_script('202008301653_Update_Merge.sql') FROM dual
;